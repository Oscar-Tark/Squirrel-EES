                ...
                else
                {
                    ...
                    if (req.Url.AbsolutePath != "/")
                    {
                        ...
                        if (Array.IndexOf(accept, HTTPElements.HTTPElements.accept_html) > -1)
                            data = Encoding.UTF8.GetBytes(string.Format(HTTPElements.HTTPElements.developmentFormatData, structure, css, logic, HTTPElements.HTTPElements.js_framework_includers[js_framework], HTTPElements.HTTPElements.static_exentsion_includers["axios"]));
                        else if (Array.IndexOf(accept, HTTPElements.HTTPElements.accept_json) > -1)
                        {
                            Console.WriteLine("Response as JSON");
                            data = Encoding.UTF8.GetBytes(string.Format("['structure' : {0}, 'css' : {1}, 'logic' : {2}, 'js_framework' : {3}], 'axios' : {4}", structure, css, logic, HTTPElements.HTTPElements.js_framework_includers[js_framework], HTTPElements.HTTPElements.static_exentsion_includers["axios"]));
                        }
                    }
...
namespace HTTPElements
{
    public static class HTTPElements
    {
        ...
        public static string developmentFormatData =
            "<!DOCTYPE html><html><head><meta charset='UTF-8'>{3}{4}</head><body>{0} {1} {2}</body></html>";
        ...
        public static Dictionary<string, string> js_framework_includers = new Dictionary<string, string> {
            { "vue.js_development", "<script src='https://cdn.jsdelivr.net/npm/vue@2/dist/vue.js'></script>" },
            { "vue.js", "<script src='https://cdn.jsdelivr.net/npm/vue@2'></script>" }
        };

        public static Dictionary<string, string> static_exentsion_includers = new Dictionary<string, string>
        {
